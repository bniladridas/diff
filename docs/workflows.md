...
- `workflows/release.yml`
  Builds and publishes a GitHub release when a `v*` tag is pushed. The release generator validates that the tag is a `v`-prefixed Semantic Versioning 2.0.0 value, such as `v0.7.0`, `v1.0.0`, or `v1.0.0-rc.1`.
  - lockfile-based dependency install with `npm ci`
  - `npm run lint`
  - `npm run build`
...
- Git tags use the common `v` prefix, but the semantic version is the value after `v`. For example, `v0.7.0` is the tag name and `0.7.0` is the SemVer value.
