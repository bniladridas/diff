...
if (!version) {
  throw new Error("Missing release tag. Usage: generate-release-notes.ts v0.3.4 /path/to/release-notes.md");
}

if (!outputPath) {
  throw new Error("Missing output path. Usage: generate-release-notes.ts v0.3.4 /path/to/release-notes.md");
}

if (!tag.startsWith("v") || !semverPattern.test(version)) {
  throw new Error(`Release tag must be v-prefixed SemVer, for example v0.3.4. Received: ${tag}`);
}
